...
class GlobalRadar:
    ...
    _WRITING_PROMPT = (
        ...
        "۴. تیتر (title_fa) حداکثر ۱۰ کلمه، جذاب و بدون کلمات اضافه. تیتر باید یک جمله‌ی کامل و درست دستوری فارسی باشه — حتماً فعل داشته باشه و فعلش با فاعل و زمان جمله درست صرف شده باشه (نه یک عبارت ناقص بدون فعل، و نه فعل با صرف غلط). "
        "فاعل و فعل تیتر باید با هم جور باشن — تیتر رو با یه موضوع/عبارت شروع نکن که فعل آخر جمله در واقع به یه فاعل دیگه برمی‌گرده (مثلاً غلط: «بازسازی X پس از استعفای مدیر، توسعه‌دهندگان عوض شد» — اینجا «بازسازی» فاعل جمله نیست ولی انگار قراره باشه؛ درست: «توسعه‌دهندگان بازسازی X پس از استعفای مدیر عوض شدند» یا یه ساختار یک‌دست دیگه). "
        "وقتی اسم خاص انگلیسی/لاتین باید با یه کلمه‌ی فارسی به‌صورت اضافه (ezafe) ترکیب بشه، ترتیب فارسی رو رعایت کن: کلمه‌ی فارسی اول، اسم خاص بعدش (مثلاً درست: «استودیوی Arkane»، غلط: «Arkane استودیوی»). "
        "غلط املایی نداشته باش (مثلاً «فروشت» به‌جای «فروش» یه غلط املاییه، نه یه سبک نوشتاری). "
        "قبل از نهایی‌کردن تیتر، خودت یک بار جمله رو از نظر دستوری، ترتیب کلمات، و املا بررسی کن.\n"
    ...
    _TITLE_QA_PROMPT = (
        "تو یک ویراستار فارسی هستی. فقط یک کار داری: تیتر زیر رو از نظر دستور زبان، املا و روانی بررسی کن.\n"
        "مشکلاتی که باید پیدا کنی:\n"
        "- فعل نداشتن، یا فعل با صرف غلط (زمان/شخص اشتباه)\n"
        "- عدم تطابق فاعل و فعل — مثلاً تیتر با یه موضوع شروع میشه ولی فعل آخر به فاعل دیگه‌ای برمی‌گرده (جمله‌ی دورهم‌ریخته)\n"
        "- غلط املایی در کلمات معمولی فارسی (نه اسامی خاص) — مثلاً «فروشت» به‌جای «فروش»\n"
        "- ترتیب غلط اضافه (ezafe) وقتی یه اسم خاص انگلیسی/لاتین با یه کلمه‌ی فارسی ترکیب شده — کلمه‌ی فارسی باید اول بیاد (درست: «استودیوی Arkane»، غلط: «Arkane استودیوی»)\n"
        "- جمله‌ی ناقص یا نامفهوم\n"
        "اگه تیتر از همه‌ی این جهات درست و روان بود، همون رو بدون هیچ تغییری برگردون.\n"
        "اگه ایراد داشت، فقط حداقل تغییر لازم برای درست‌کردنش رو بده — معنا و طول تیتر رو حفظ کن، بازنویسی کامل نکن.\n"
        "🚫 هرگز خودِ اسم خاص (اسم شخص، شرکت، مکان) رو عوض نکن، حتی اگه فکر می‌کنی یه املای رایج‌تر یا آشناتر هست — فقط می‌تونی جای اسم خاص رو توی جمله (برای رعایت ترتیب اضافه) جابه‌جا کنی، نه خودش رو تغییر بدی.\n"
